...
class schoolReport extends Component {
    ...
    createTable=()=>{
        ...
        for(let i=0;i<this.state.schools.length;i++){
            let nextPage=null;
            var pageLink=null;
            if(this.state.schools[i].schoolStatus==="ProjectClosed"){
                nextPage="adminDEOtrigger"
                pageLink="Edit/Delete"
            }
            if(this.state.schools[i].schoolStatus==="SCHOOL_REGISTERED"){
            ...
            if(this.state.schools[i].schoolStatus==="SCHOOL_REGISTERED" || this.state.schools[i].schoolStatus==="ProjectClosed" ||
             this.state.schools[i].schoolStatus==="DEO_REJECTED"){
                rowsUpdated=true;
                rows.push(<tr>
...
export default withRouter(schoolReport);